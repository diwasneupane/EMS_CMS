...
export default function TeachersPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      usersApi.getTeachers({ page, limit: 10, search: debouncedSearch, departmentId: deptFilter || undefined }),
  });
  ...
  const openCreate = () => {
    setEditItem(null);
    reset({ firstName: '', lastName: '', email: '', password: '', phoneNumber: '', departmentId: '' });
    setModalOpen(true);
  };
  ...
  const columns: Column<User>[] = [
    {
      header: 'Teacher', accessor: 'firstName', render: (_, row) => (
    ...
    {
      header: 'Phone', accessor: 'phoneNumber', render: (_, row) => (
        <span className="text-sm text-slate-600">{row.phoneNumber ?? '-'}</span>
      ),
    },
    {
      header: 'Status', accessor: 'role', render: () => (
