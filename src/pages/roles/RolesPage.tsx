...
export default function RolesPage() {
  ...
  const openEdit = (item: Role) => {
    setEditItem(item);
    const currentPermIds = (item.rolePermissions ?? []).map(
      (rp) => rp.permissionId,
    );
  ...
  const onSubmit = (formData: FormData) => {
    ...
    if (editItem) {
      updateMutation.mutate(
        ...
        {
          onSuccess: () => {
            // ✅ Always sync permissions, even if empty (means "remove all")
      ...
      render: (_, row) => {
        ...
        return (
          <div className="flex flex-wrap gap-1">
            {shown.map((rp) => {
              const colorClass =
                RESOURCE_COLORS[rp.permission.resource] ??
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex items-center gap-1">
          <button
            onClick={() =>
              setExpandedRole(expandedRole === row.id ? null : row.id)
            }
          ...
          >
            <ChevronDown
              className={cn(
                ...
                expandedRole === row.id && "rotate-180",
              )}
            />
          </button>
          {can("roles", "update") && (
            ...
            {allPermissions.length === 0 ? (
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden max-h-72 overflow-y-auto">
                {Object.entries(grouped).map(([resource, perms]) => {
                  ...
                  const colorClass =
                    RESOURCE_COLORS[resource] ?? "bg-slate-50 text-slate-600";
                  return (
                    <div
                      key={resource}
